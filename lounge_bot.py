...
# --- команды и панель для тебя/менеджеров ---
def admin_panel_kb() -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    b.button(text="📊 Статистика", callback_data="adm:stats")
    b.button(text="💰 Неоплаченные", callback_data="adm:unpaid")
    b.button(text="📋 Все заявки", callback_data="adm:orders")
    b.adjust(1)
    return b.as_markup()


def is_admin_ids(user_id: int, chat_id: int) -> bool:
    return chat_id == MANAGER_GROUP_ID or user_id in ADMIN_IDS


async def user_breakdown_lines():
    """Разбивка по клиентам: каждый заказ и итог по клиенту."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute(
            "SELECT id, user_id, username, persons, total, status FROM orders "
            "WHERE status != 'rejected' ORDER BY user_id, id"
        )
        rows = await cur.fetchall()
    if not rows:
        return ["Заявок пока нет."]
    lines = ["👥 <b>По клиентам</b>\n"]
    cur_uid = None
    user_sum = 0
    grand = 0
    def flush():
        ...
        if cur_uid is not None:
            lines.append(f"<b>сумма: {user_sum}₽</b>")
            lines.append("")
        user_sum = 0
    for o in rows:
        if o["user_id"] != cur_uid:
            flush()
            cur_uid = o["user_id"]
            name = f"@{o['username']}" if o["username"] else f"id {o['user_id']}"
            lines.append(f"👤 {name}")
        mark = "✅" if o["status"] == "paid" else ("🙋" if o["status"] == "in_work" else "🆕")
        lines.append(f"{mark} #{o['id']}: {o['persons']} прох. = {o['total']}₽")
        user_sum += o["total"] or 0
        grand += o["total"] or 0
    flush()
    lines.append("—")
    lines.append(f"💰 Общая сумма: <b>{grand}₽</b> (✅ оплачено / 🙋 в работе / 🆕 новая)")
    return lines


async def stats_lines():
    ...
    return head + await user_breakdown_lines()


async def unpaid_lines():
    rows = await list_orders(only_unpaid=True)
    if not rows:
        return ["Нет неоплаченных заявок 🎉"]
...
async def cmd_admin(message: Message):
    if not is_admin(message):
        return
    await message.answer("🛠 <b>Панель менеджера</b>", reply_markup=admin_panel_kb())


@router.callback_query(F.data.startswith("adm:"))
async def cb_admin(cb: CallbackQuery):
    if not is_admin_ids(cb.from_user.id, cb.message.chat.id):
        await cb.answer("Нет доступа", show_alert=True)
        return
    action = cb.data.split(":")[1]
    if action == "stats":
        lines = await stats_lines()
    elif action == "unpaid":
        lines = await unpaid_lines()
    else:
        lines = await orders_lines()
    await send_lines(cb.message, lines)
    await cb.message.answer("🛠 Панель:", reply_markup=admin_panel_kb())
    await cb.answer()
...
async def cmd_stats(message: Message):
    if not is_admin(message):
        return
    await send_lines(message, await stats_lines())
...
async def cmd_unpaid(message: Message):
    if not is_admin(message):
        return
    await send_lines(message, await unpaid_lines())
...
async def cmd_orders(message: Message):
    if not is_admin(message):
        return
    await send_lines(message, await orders_lines())
